...
import { musingTable } from "#/db/schema";

type MusingRow = typeof musingTable.$inferSelect;
type MusingInsert = typeof musingTable.$inferInsert;
	...
	implements MusingsCommandPort, MusingsQueryPort
{
	...
	async get(input: Parameters<MusingsQueryPort["get"]>[0]) {
		const [row] = await db
			.select()
			.from(musingTable)
			.where(eq(musingTable.id, toNumericId(input.id)))
			.limit(1);
	...
	async list(
		...
		const rows = await db
			.select()
			.from(musingTable)
			.orderBy(desc(musingTable.createdAt), desc(musingTable.id))
			.limit(limit + 1)
			.offset(offset);
	...
	async listPublished(
		...
		const rows = await db
			.select()
			.from(musingTable)
			.where(isNotNull(musingTable.publishedAt))
			.orderBy(asc(musingTable.seq), asc(musingTable.id))
			.limit(limit + 1)
			.offset(offset);
