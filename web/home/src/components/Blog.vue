    ...
    <div id="blog-contents" v-else>
      <article>
        <div class="inner-title-container">
          <h1>
            <a :href="address" id="inner-title">{{ title }}</a>
          </h1>
      ...
      <div id="adjacent-articles">
        <div id="next-article" v-if="currentArticleIdx > 0">
          <h4>Next Article</h4>
          <p>
            <router-link :to="index[currentArticleIdx-1].path">{{index[currentArticleIdx-1].title}}</router-link>
          </p>
        </div>
...
export default {
  ...
  {
    "relativeId": 14,
    "title": "[macOS] 맥 터미널로 우분투 사용하기",
    "path": "/blog/2016/11/02/macos-맥-터미널로-우분투-사용하기/",
    "date": {
      "year": "2016",
      "month": "11",
      "monthEng": "November",
